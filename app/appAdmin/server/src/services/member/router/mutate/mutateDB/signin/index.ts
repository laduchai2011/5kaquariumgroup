...
import { MutateDB } from '@src/services/member/interface';
import { signin_infor_type } from '../../handle/signin/type';
import { MemberField } from '@src/dataStruct/member';
...
class MutateDB_Signin extends MutateDB {
    ...
    async run(): Promise<sql.IResult<MemberField> | undefined> {
        if (this._connectionPool !== undefined && this._siggnin_infor !== undefined) {
            try {
                const result = await this._connectionPool
                    ...
                    .query(`SELECT * FROM dbo.Signin_Member(@userName, @password)`);
