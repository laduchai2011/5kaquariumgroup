...
import QueryDB_Get_Statistic from '../../queryDB/GetStatistic';
import { MyResponse } from '@src/dataStruct/response';
import { StatisticField } from '@src/dataStruct/account';
import { verifyRefreshToken } from '@src/token';


class Handle_Get_Statistic {
    ...
    main = async (req: Request, res: Response) => {
        ...
        const queryDB_get_statistic = new QueryDB_Get_Statistic();

        if (userId && !isNaN(Number(userId)) && userId !== -1) {
            queryDB_get_statistic.setUserId(Number(userId))
        } else {
        ...
        if (conn) {
            queryDB_get_statistic.set_connection_pool(conn)
        } else {
            myResponse.message = 'Kết nối cơ sở dữ liệu không thành công !';
            return res.status(500).json(myResponse);
        }

        const result = await queryDB_get_statistic.run();
...
export default Handle_Get_Statistic;
