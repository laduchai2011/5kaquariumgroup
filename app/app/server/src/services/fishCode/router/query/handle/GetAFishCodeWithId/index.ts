...
import QueryDB_Get_AFishCodeWithId from '../../queryDB/GetAFishCodeWithId';
import { MyResponse } from '@src/dataStruct/response';
import { FishCodeField } from '@src/dataStruct/fishCode';
import { verifyRefreshToken } from '@src/token';


class Handle_Get_AFishCodeWithId {
    ...
    main = async (req: Request<unknown, unknown, unknown, {id: string}>, res: Response) => {
        ...
        const queryDB_get_aFishCodeWithId = new QueryDB_Get_AFishCodeWithId();

        if (id !== -1) {
            queryDB_get_aFishCodeWithId.setId(id);
        } else {
            myResponse.message = 'Id mã cá không chính xác !';
            return res.status(500).json(myResponse);
        }

        const conn = this._mssql_server.get_connectionPool();
        if (conn) {
            queryDB_get_aFishCodeWithId.set_connection_pool(conn)
        } else {
            myResponse.message = 'Kết nối cơ sở dữ liệu không thành công !';
            return res.status(500).json(myResponse);
        }

        const result = await queryDB_get_aFishCodeWithId.run();
...
export default Handle_Get_AFishCodeWithId;
